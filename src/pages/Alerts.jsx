  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div
          className={`w-11 h-11 ${cfg.iconBg} rounded-xl flex items-center justify-center text-xl flex-shrink-0`}
        >
          {cfg.emoji}
        </div>
        <div className="flex-1">
          ...
          <div className="flex gap-2.5 mt-4">
            <button
              ...
              className={`text-xs text-white font-bold px-4 py-2.5 rounded-xl transition-colors shadow-sm cursor-pointer ${cfg.btn}`}
            >
              View Full Profile
            </button>
            <button
              onClick={() => onToast?.(`${prospect.name} marked as called`)}
              className="text-xs bg-slate-100 text-slate-700 font-semibold px-4 py-2.5 rounded-xl hover:bg-slate-200 transition-colors cursor-pointer"
            >
              Mark as Called
            </button>
            {type === 'cold' && (
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g.type} className="space-y-3">
            <div
              className={`text-xs font-bold uppercase tracking-widest px-1 ${cfg.sectionColor}`}
            >
              {cfg.section}
            </div>
            {g.prospects.map((p) => (
